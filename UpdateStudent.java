...
public class UpdateStudent extends JFrame implements ActionListener {
    ...
    UpdateStudent(){
        ...
        heading.setBounds(260,10,280,30);
        ...
        allID = new JLabel("Select Roll No. :");
        allID.setBounds(135,75,150,30);
        ...
        try{
            ...
            while(rs.next()){
                AllID.addItem(rs.getString("roll"));
            }
        }catch (Exception E){
            E.printStackTrace();
        }
    ...
    public void actionPerformed(ActionEvent e){
        ...
        if(e.getSource()==Search){
            try{
                ResultSet rs = JDBC.jdbcgetConnection().prepareStatement("select * from student where roll = '"+AllID.getSelectedItem()+"'").executeQuery();
                if(rs.next()){
                    Name.setText(rs.getString("name"));
                    FatherName.setText(rs.getString("father"));
                    EmployeeId.setText(rs.getString("roll"));
                    ...
                    Qualification.setSelectedItem(rs.getString("course"));
                    Department.setSelectedItem(rs.getString("branch"));
                }
            }catch (Exception E){
                E.printStackTrace();
            }
        ...
        if(e.getSource()==Submit){
            if(EmployeeId.getText().equals("")){
                JOptionPane.showMessageDialog(this,"Data Field Is Empty! Confirm To Select " +
                        "Student Roll Number Than Search.","Warning",JOptionPane.WARNING_MESSAGE);
            }
            else{
                try{
                    String s = "update student set address = '"+Address.getText()+"', phone = '"+Phone.getText()+"', email = '"+Email.getText()+"', course = '"+Qualification.getSelectedItem()+"', branch = '"+Department.getSelectedItem()+"' where roll = '"+EmployeeId.getText()+"'";
