...
public class StudentLeave extends JFrame implements ActionListener {
    ...
    StudentLeave(){
        ...
        heading = new JLabel("Student Leave Form!");
    ...
    public void actionPerformed(ActionEvent e){
        ...
        if(e.getSource()==Submit){
            if(((JTextField)LeaveDate.getDateEditor().getUiComponent()).getText().equals("")||
                ...
                try{
                    int x = JDBC.jdbcgetConnection().prepareStatement("insert into studentLeave values('"+Search.getSelectedItem()+"', '"+((JTextField)LeaveDate.getDateEditor().getUiComponent()).getText()+"', '"+((JTextField)LeaveDateEnd.getDateEditor().getUiComponent()).getText()+"', '"+TimeDuration.getSelectedItem()+"')").executeUpdate();
                    if(x>0){
                        dispose();
                    }
                }catch(Exception E){
